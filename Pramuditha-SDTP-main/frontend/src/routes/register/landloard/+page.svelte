<script lang="ts">
	...
	async function handleSubmit(event: SubmitEvent) {
		...
		if (browser) {
			const response = await fetch('http://localhost:8080/api/register_landloard.php', {
				method: 'POST',
				body: formData
			});
...
<svelte:head>
	<title>Hostel Finder - Landloard Register</title>
</svelte:head>
