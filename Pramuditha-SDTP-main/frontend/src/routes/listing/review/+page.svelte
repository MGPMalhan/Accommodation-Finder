<script lang="ts">
	...
	const onFilter = () => {
		...
		const url = `/listing/review?page=${currentPage}&location=${selectedLocation}&minPrice=${selectedPricingMin}&maxPrice=${selectedPricingMax}`;
		window.location.replace(url);
	};

	const changePage = (page: Number) => {
		const url = `/listing/review?page=${page}`;
		window.location.replace(url);
	};
	...
	const acceptSubmission = (id: Number) => {
		// Fetch to localhost:8080/api/listing/accept.php
	...
	const rejectSubmission = (id: Number) => {
		// Fetch to localhost:8080/api/listing/reject.php
		fetch(`http://localhost:8080/api/listing/reject.php?listingId=${id}`)
			.then((response) => response.text())
			.then((result) => {
				console.log(result);
				if (result.includes('Error')) {
...
<svelte:head>
	<title>Hostel Finder - Listing Review</title>
</svelte:head>
		...
		{#each cards as card}
			<div class="flex flex-col justify-between overflow-hidden rounded shadow-lg">
				...
				<div class="grid grid-cols-2 gap-4 px-6 py-4">
					<button
						on:click={() => {
							acceptSubmission(card.id);
						}}
						class="bg-primary-500 hover:bg-primary-700 rounded-md px-4 py-2 text-white transition duration-500 ease-in-out"
					>
						Accept
					</button>
					<button
						on:click={() => {
							rejectSubmission(card.id);
						}}
						class="rounded-md bg-rose-600 px-4 py-2 text-white transition duration-500 ease-in-out hover:bg-red-700"
					>
						Reject
					</button>
				</div>
